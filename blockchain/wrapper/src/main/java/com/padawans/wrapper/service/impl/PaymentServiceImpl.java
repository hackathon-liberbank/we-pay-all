...
public class PaymentServiceImpl implements PaymentService {
    ...
    public ResponseEntity<PaymentResponse> paymentsPaymentIDPost(String paymentID, PaymentRequest payment) {
        DemoBank contract = service.getDemoBank()
                .orElseThrow(() -> new SmartContractException("No se ha podido recurar el smart contract"));
        try {
            TransactionReceipt tx = contract.addPaymentOrder(new BigInteger(payment.getEventID().toString()),
                    new BigInteger(paymentID), new BigInteger(payment.getBeneficiary().toString()),
                    new BigInteger(payment.getAmount().toString())).send();
