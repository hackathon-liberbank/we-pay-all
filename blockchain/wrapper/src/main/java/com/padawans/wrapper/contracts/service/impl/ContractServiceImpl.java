...
public class ContractServiceImpl implements ContractService {
    ...
    public TransactionReceipt getTransactionReceipt(String txHash) {
        try {
            TransactionReceipt transactionReceipt = web3j.ethGetTransactionReceipt(txHash).send()
                    .getTransactionReceipt()
                    .orElseThrow(() -> new SmartContractException("No existe informacion de transaccion"));
            return transactionReceipt;
        } catch (IOException e) {
            throw new SmartContractException(
                    "No se ha podido recuperar La informacion de transaccion. Error: " + e.getMessage());
        }
